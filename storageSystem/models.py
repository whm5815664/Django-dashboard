...
class Device(models.Model):
    ...
    # devices.device_name / device_code
    name = models.CharField("设备名称", max_length=64, db_column="device_name", unique=True)
    code = models.CharField("设备编号", max_length=64, db_column="device_code", null=True, blank=True)

    # 关键：字段名叫 cold_room（兼容你现有业务语义），但映射到真实列 base_id
    # 绝对不要再用 ForeignKey，否则会去找 coldroom_id
    cold_room = models.CharField("所属冷库ID", max_length=10, db_column="base_id", null=True, blank=True)

    # devices.longitude / latitude / location
    longitude = models.DecimalField("经度", max_digits=10, decimal_places=6, db_column="longitude", null=True, blank=True)
    latitude = models.DecimalField("纬度", max_digits=10, decimal_places=6, db_column="latitude", null=True, blank=True)
    location = models.CharField("位置信息", max_length=255, db_column="location", null=True, blank=True)

    # devices.status enum('online','offline','alarm')
