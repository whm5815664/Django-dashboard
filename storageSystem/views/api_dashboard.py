...
def _serialize_device(obj: Device) -> Dict[str, Any]:
    ...
    return {
        ...
        "cold_room": _get_fk_id(obj, DEVICE_COLD_F),
...
def dashboard_devices(request):
    ...
    try:
        ...
        # cold_room 过滤
        if cold_room and cold_room.isdigit() and DEVICE_COLD_F:
            qs = qs.filter(**{f"{DEVICE_COLD_F}_id": int(cold_room)})
...
def update_device(request):
    ...
    Content-Type: application/json
    {
      ...
      "device_name": "新名称",
      "device_code": "SN001",
      "coldroom_id": 1,      # 兼容 cold_room_id / cold_room
      "longitude": 113.1,
      "latitude": 30.4,
      "location": "武汉",
      "status": "online"     # online/offline/alarm/normal
    }
    """
    body = _parse_json_body(request)
    data: Dict[str, Any] = {}
    ...
    try:
        ...
        # coldroom_id / cold_room_id / cold_room
        if _key_in(data, "coldroom_id", "cold_room_id", "cold_room") and DEVICE_COLD_F:
            ...
            fk_id_attr = f"{DEVICE_COLD_F}_id"
            if hasattr(obj, fk_id_attr):
                setattr(obj, fk_id_attr, cid)
                update_fields.append(DEVICE_COLD_F)
            else:
                # 如果不是 FK（极少数情况）
                setattr(obj, DEVICE_COLD_F, cid)
                update_fields.append(DEVICE_COLD_F)
